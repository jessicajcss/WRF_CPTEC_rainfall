...
# ---- 6d) Export to GeoJSON + Shapefile ----
#
# output_dir : folder to write files into
# run_dt     : POSIXct of the run init (used in filename)
# df_multi   : result of extract_rain_multi_cep()
...
export_spatial <- function(df_multi, run_dt,
  ...
  # Build sf object: one row per CEP × hour
  df_out <- df_multi |>
    ...
    dplyr::select(cep, label, lon, lat,
                  init_utc  = init_utc_str,
                  valid_utc = valid_utc_str,
                  forecast_hour, accum_mm, rain_mm, rain_class)

  sf_out <- sf::st_as_sf(df_out, coords = c("lon","lat"), crs = 4326, remove = FALSE)

  written <- character(0)

  # GeoJSON — best for Lovable and R/leaflet
  if ("geojson" %in% formats) {
    gj_path <- file.path(output_dir, paste0(stem, ".geojson"))
    sf::st_write(sf_out, gj_path, driver = "GeoJSON",
                 layer_options = "RFC7946=YES",
                 delete_dsn = TRUE, quiet = TRUE)
    written <- c(written, gj_path)
    if (verbose) cat(sprintf("GeoJSON : %s  (%d features, %d CEPs x %d hours)\n",
                             gj_path, nrow(sf_out),
                             length(unique(df_out$cep)),
                             length(unique(df_out$forecast_hour))))
  }

  # Shapefile — best for QGIS
  if ("shp" %in% formats) {
    ...
    # Shapefile has 10-char column name limit — rename long cols
    sf_shp <- sf_out |>
      dplyr::rename(init_dt  = init_utc,
                    valid_dt = valid_utc,
                    fcst_hr  = forecast_hour,
                    rain_cls = rain_class)

    sf::st_write(sf_shp, shp_path, delete_dsn = TRUE, quiet = TRUE)
    written <- c(written, shp_path)
    if (verbose) cat(sprintf("Shapefile: %s\n", shp_path))
  }

  if (verbose) {
    cat("\nRain intensity summary across all CEPs and hours:\n")
    print(table(df_out$rain_class))
    cat("\nHourly rainfall range (mm):\n")
    cat(sprintf("  min=%.3f  median=%.3f  max=%.3f\n",
                min(df_out$rain_mm, na.rm=TRUE),
                median(df_out$rain_mm, na.rm=TRUE),
                max(df_out$rain_mm, na.rm=TRUE)))
  }
...
# Columns: municipio, bairro, logradouro, cep, cep_fmt, label, lon, lat
...
cat(sprintf("  Sample:\n"))
print(head(cep_df[, c("municipio","localidade","cep_fmt","lon","lat")], 5))
