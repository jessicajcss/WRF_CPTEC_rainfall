...
extract_rain_cep <- function(run_dir, cep, verbose=TRUE) {
  if (verbose) cat("Discovering files...\n")
  meta <- discover_wrf_files(run_dir)

  if (verbose) cat("Geocoding CEP...\n")
  cep_digits <- gsub("\\D","",cep)
  if (nchar(cep_digits) != 8) stop("CEP must have 8 digits.")
  geo <- geocodebr::busca_por_cep(cep_digits)
  if (!nrow(geo)) stop("CEP not found: ", cep)
  ...
  pt <- terra::vect(
    data.frame(lon=as.numeric(geo[[lon_col]][1]), lat=as.numeric(geo[[lat_col]][1])),
    geom=c("lon","lat"), crs="EPSG:4674")
...
## ---- 0b) Target location ----
#MUNICIPIO_NAME <- "Joinville"   # used when OUTPUT_MODE == "municipality"
#CEP            <- "89239-100"   # used when OUTPUT_MODE == "cep"
#OUTPUT_MODE <- "cep"
...
if (OUTPUT_MODE == "municipality") {
  ...
  df <- extract_rain_cep(run_dir, CEP, verbose=TRUE)
} else {
